def security_enabled?
  ...
  file = Tempfile.new('is_security_enabled')
  file.write script
  file.flush
  file.close! if file
...
def check_security_flag
  if ::File.exists?('/var/lib/jenkins/security_enabled_by_chef')
    ...
    if r == 'true'
      Chef::Log.warn 'True'
      node.run_state[:jenkins_username] = node['bluefinops_jenkins']['user']
      node.run_state[:jenkins_password] = node['bluefinops_jenkins']['user']
    end
   end
end
...
action :install_plugin do
  ...
  if (security_enabled?)
    ...
    node.run_state[:jenkins_password] = 'jenkins'
  end
  jenkins_plugin n do
    action :install
